...
아래 길이 정보는 2026-03-30 기준 운영 DB에 probe 데이터를 넣어 검증한 값입니다.

```sql
CREATE TABLE defects (
    defect_id       BIGINT PRIMARY KEY,          -- 결함 ID (Unique Timestamp)
    title           VARCHAR(255) NOT NULL,       -- 결함 제목
    ...
    env_info        VARCHAR(255),                -- 테스트 환경 정보 (Browser/OS)
...
CREATE TABLE defect_save_error_logs (
    ...
    reported_by      VARCHAR(50),               -- 등록자명 (payload creator)
...
*   **등록자/작업자 분리**: `reported_by`에는 결함 등록자명을 저장하고, 실제 저장 시점 작업자명은 `extra.reporter_name`에 함께 저장합니다.
...
*   **운영 DB 검증값**: `title/menu_name/screen_name/env_info/creator/assignee`는 운영 DB에서 `VARCHAR(255)`로 확인되었고, `screen_url/steps_to_repro/screenshot/action_comment`는 `TEXT` 계열로 사용됩니다.
